...
The existing apparatus seems capable of an even more compelling quantum
...
\subsection*{MZI Operator}

We previously created the MZI operator $\hat{Z}'(\vartheta)$
where $\vartheta$ is the MZI HWP$_t$ angle.
Let's compose a polarizer operator for the idler polarizer
$LP_i$ onto $\hat{Z}'$
to create an operator for the full single photon quantum eraser experiment.
...
\paragraph{Operator for a single photon}
And now we can compose $\hat{P'}$ and $\hat{Z}'$
to create an operator that encompasses all of the single photon quantum eraser
apparatus:
...
First, let's apply the single photon eraser operator to the idler photon:
\[
\hat{\mathcal{E}}=
