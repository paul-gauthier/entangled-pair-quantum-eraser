...
\begin{tikzpicture}[scale=1.8]
  ...
  \node[below=0.25cm of HWP] () {\small HWP($45^\circ$)};
    ...
    \node at ($(BS2)-(0.4,0.4)$) {\small BS$_2$};
    ...
    \node[left=0.5cm of LPs] () {\small LP$_s(\varphi)$};
  ...
  \node[above=0.25cm of PM,align=center] () {\small Power meter\\(retractable)};
     ...
     \node[above=0.1cm of MZI_HWP] () {\small HWP($45^\circ$)};
     \node[hwp,rotate=0,right=1.25cm of BS1] (MZI_HWP2) {};
     \node[below=0.25cm of MZI_HWP2] () {\small HWP($0^\circ$)};

\end{tikzpicture}
\caption{
Entangled signal-idler quantum eraser experiment, where the signal photon
carries the ``which-way'' information about the idler's path through the MZI.
The apparatus is unchanged from Lab 6.
Like in Lab 6, the MZI HWP in the upper arm is set to $45^\circ$
and the MZI LP at the horizontal output by $D_i$
is set to $90^\circ$.
The only changes are: 1. Set the pump HWP to $45^\circ$ to generate entangled pairs;
2. Vary the signal LP angle $\varphi$ to control the eraser.
Setting $\varphi = 45^\circ$ erases the ``which-way'' information from the signal,
while $\varphi \in \{0^\circ, 90^\circ\}$ retains it.
\label{fig:apparatus}
}
\end{figure*}
