...
def main():
    ...
    print(
        f"{'hwpoff':>6s}, {'lpoff':>6s}, {'phi(hwp0)':>10s}, {'phi(hwp45)':>10s},"
        f" {'phi(hwp45)-90':>14s}"
    )
    for (hwpoff, lpoff), hwp_files in sorted(file_groups.items()):
        ...
        print(
            f"{hwpoff:6.1f}, {lpoff:6d}, {phi0_str:>10s}, {phi45_str:>10s},"
            f" {phi45_minus_90_str:>14s}"
        )

        # Store data for later analysis
        if not np.isnan(phi0):
            phi0_data.append((hwpoff, phi0))
        if not np.isnan(phi45):
            phi45_data.append((hwpoff, phi45))

    # Finished processing all file groups
    return phi0_data, phi45_data
